...
def parseDEF():
    ...
    next_token()
    holder = SymbolExpr().eval(True)   # delete if bug
    
    if next_token() =="=":
        
        dict1[holder]= parseExpr()
        return dict1[holder]
    else:
        raise ParseError( "Expecting '=' after def symbol ", tok )
...
class parseLET:
  ...
  def parseVlist( self ):
        ...
        if tok in bad_expr:
            raise ParseError( "Invalid expression", tok )
        rule( "VLIST -> SYMBOL '=' EXPR V_TAIL" )

        holder = SymbolExpr().eval(True)
      
        if next_token() =="=":
            self.dict2[holder]= parseExpr().eval(True)
            #print (self.dict2)
        else:
            raise ParseError( "Expecting '=' after def symbol ", tok )
        self.parseVTail()
...
class UnaryExpr:
  ...
  def eval( self, env ):
    ...
    if func.typ != None and type( acc ) != func.typ:
      raise EvalError( "Operand types are not correct", self.op.line, 
                                                        self.op.col )
...
class SymbolExpr:
  def __init__( self ):
    ...
    if self.sym in dict1:
        rule("1 check")
        self.sym=dict1[self.sym].eval(True)
    elif self.sym in parseLET.dict2:
        rule("2 check")
        self.sym=parseLET.dict2[self.sym]
